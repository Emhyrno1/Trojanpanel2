...
init_var() {
  ...
  # Caddy
  CADDY_DATA="/tpdata2/caddy/"
  CADDY_Config="/tpdata2/caddy/config.json"
  CADDY_SRV="/tpdata2/caddy/srv/"
  CADDY_CERT="/tpdata2/caddy/cert/"
  CADDY_LOG="/tpdata2/caddy/logs/"
  ...
  # Trojan Panel
  TROJAN_PANEL_DATA="/tpdata2/trojan-panel/"
  TROJAN_PANEL_WEBFILE="/tpdata2/trojan-panel/webfile/"
  TROJAN_PANEL_LOGS="/tpdata2/trojan-panel/logs/"

  # Trojan Panel UI
  TROJAN_PANEL_UI_DATA="/tpdata2/trojan-panel-ui/"
  # Nginx
  ...
  # Trojan Panel Core
  TROJAN_PANEL_CORE_DATA="/tpdata2/trojan-panel-core/"
  TROJAN_PANEL_CORE_LOGS="/tpdata2/trojan-panel-core/logs/"
      ...
      if [[ -z ${ssl_option} || ${ssl_option} == 1 ]]; then
        while read -r -p "请选择申请证书的方式(1/acme 2/zerossl 默认:1/acme): " ssl_module_type; do
          if [[ -z "${ssl_module_type}" || ${ssl_module_type} == 1 ]]; then
            ...
            ssl_module="zerossl"
            CADDY_CERT_DIR="/tpdata2/caddy/cert/certificates/acme.zerossl.com-v2-dv90/"
            break
          else
...
EOF
    ...
    docker pull caddy:2.6.2 &&
      docker run -d --name trojan-panel-caddy-2 --restart always \
        --network=host \
  ...
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-mariadb$") ]]; then
    ...
    if [[ "${mariadb_user}" == "root" ]]; then
      docker pull mariadb:10.7.3 &&
        docker run -d --name trojan-panel-mariadb-2 --restart always \
          --network=host \
...
install_redis() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-redis$") ]]; then
    ...
    docker pull redis:6.2.7 &&
      docker run -d --name trojan-panel-redis-2 --restart always \
        --network=host \
        redis:6.2.7 \
...
install_trojan_panel() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel$") ]]; then
    ...
    docker pull jonssonyan/trojan-panel:2.0.5 &&
      docker run -d --name trojan-panel-2 --restart always \
        --network=host \
...
EOF
    ...
    docker pull jonssonyan/trojan-panel-ui:2.0.4 &&
      docker run -d --name trojan-panel-ui-2 --restart always \
        --network=host \
        -v "${NGINX_CONFIG}":"/etc/nginx/conf.d/default.conf" \
        -v ${CADDY_CERT}:${CADDY_CERT} \
        jonssonyan/trojan-panel-ui:2.0.4
...
install_trojan_panel_core() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-core$") ]]; then
    ...
    docker pull jonssonyan/trojan-panel-core:2.0.4 &&
      docker run -d --name trojan-panel-core-2 --restart always \
        --network=host \
        ...
        jonssonyan/trojan-panel-core:2.0.4
...
update_trojan_panel() {
  ...
  if [[ "${trojan_panel_current_version}" != "${trojan_panel_latest_version}" ]]; then
    ...
    docker rm -f trojan-panel-ui &&
      docker rmi -f jonssonyan/trojan-panel-ui:2.0.4 &&
...
update_trojan_panel_core() {
  ...
  if [[ "${trojan_panel_core_current_version}" != "${trojan_panel_core_latest_version}" ]]; then
    ...
    docker rm -f trojan-panel-core &&
      docker rmi -f jonssonyan/trojan-panel-core:2.0.4
